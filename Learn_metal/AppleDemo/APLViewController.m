...
- (void)imagePickerController:(APLImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    NSURL *locationUrl = [NSURL fileURLWithPath:[self getLocationPath]];
    [videoData writeToURL:locationUrl atomically:YES];
    
	[self setupPlaybackForURL:locationUrl];
...
{
    NSString *directoryPath = [NSString stringWithFormat:@"%@/Video",NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).lastObject];
    NSFileManager *manager = [[NSFileManager alloc] init];
    BOOL isDire;
    if (![manager fileExistsAtPath:directoryPath isDirectory:&isDire]) {
    ...
    NSString *path = [directoryPath stringByAppendingPathComponent:@"test.mp4"];
